...
For normal use, install the CLI with `npm install --global @agenthouse/engineering --ignore-scripts`, then run `ah-engineering onboard --root /path/to/project`. Each application is enrolled separately; no framework checkout is needed. To pin the CLI, install `@agenthouse/engineering@0.1.8` instead.

For a project-local CLI, use `npm install --save-dev --save-exact @agenthouse/engineering@0.1.8 --ignore-scripts`, then `npx --no-install ah-engineering onboard`. This updates the application's package manifest and lockfile; use the global option if you do not want a development dependency. A supplied archive can be installed with `npm install --global /path/to/agenthouse-engineering-0.1.8.tgz --ignore-scripts`. npm's registry, proxy and certificate configuration applies when distributing through internal registries. The dependency-free tarball can be installed offline with Node/npm already provisioned.
...
Do not commit agent inspection screenshot galleries. Write throwaway captures under `artifacts/agenthouse/` or `.agenthouse/evidence/` (both ignored), inspect them, then run `node .agenthouse/run.mjs housekeep`. Durable Git coverage is the acceptance contract and reviewed Playwright snapshots/tests. Criterion findings belong on the work record; CI archives `artifacts/agenthouse/` as retained evaluation evidence. `session` applies the same housekeeping rules at task start. `doctor` reports missing ignore rules and Git-tracked inspection captures. See [ADR-0010](adr/0010-ephemeral-frontend-captures.md).
