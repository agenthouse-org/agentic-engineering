...
For normal use, install the CLI with `npm install --global @agenthouse/engineering --ignore-scripts`, then run `ah-engineering onboard --root /path/to/project`. Each application is enrolled separately; no framework checkout is needed. To pin the CLI, install `@agenthouse/engineering@1.1.0` instead.

For a project-local CLI, use `npm install --save-dev --save-exact @agenthouse/engineering@1.1.0 --ignore-scripts`, then `npx --no-install ah-engineering onboard`. This updates the application's package manifest and lockfile; use the global option if you do not want a development dependency. A supplied archive can be installed with `npm install --global /path/to/agenthouse-engineering-1.1.0.tgz --ignore-scripts`. npm's registry, proxy and certificate configuration applies when distributing through internal registries. The dependency-free tarball can be installed offline with Node/npm already provisioned.
...
Do not commit agent inspection screenshot galleries. Write throwaway captures only under `.agenthouse/evidence/<work-id>/`. Never use `tests/output` or invent another screenshot folder. Inspect them, then run `node .agenthouse/run.mjs housekeep`. Housekeep ignores and deletes leftover dump trees, including untracked `tests/output` galleries, and untracked repository-root `tmp-*` drafts such as `tmp-pr-178.md`. It does not delete Git-tracked files or evaluation reports. Durable Git coverage is the acceptance contract and reviewed Playwright snapshots/tests. Criterion findings belong on the work record; CI archives `artifacts/agenthouse/` as retained evaluation evidence. `session` applies the same housekeeping rules at task start. `doctor` reports missing ignore rules, Git-tracked inspection captures, leftover dumps, and root scratch files. See [ADR-0010](adr/0010-ephemeral-frontend-captures.md).
