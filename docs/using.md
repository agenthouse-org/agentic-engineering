...
Version 1.5.0 adds CI guidance for preserving and publishing JUnit XML reports. See [CLI evaluation and CI/CD integration](cli-evaluation.md) for GitHub and GitLab behavior. Role and product-process guidance from 1.4.0 remains available in [roles and product processes](roles-and-processes.md).
...
For normal use, install the CLI with `npm install --global @agenthouse/engineering --ignore-scripts`, then run `ah-engineering onboard --root /path/to/project`. Each application is enrolled separately; no framework checkout is needed. To pin the CLI, install `@agenthouse/engineering@1.5.0` instead.

For a project-local CLI, use `npm install --save-dev --save-exact @agenthouse/engineering@1.5.0 --ignore-scripts`, then `npx --no-install ah-engineering onboard`. This updates the application's package manifest and lockfile; use the global option if you do not want a development dependency. A supplied archive can be installed with `npm install --global /path/to/agenthouse-engineering-1.5.0.tgz --ignore-scripts`. npm's registry, proxy and certificate configuration applies when distributing through internal registries. The dependency-free tarball can be installed offline with Node/npm already provisioned.
...
Keep generated screenshots, traces and reports out of Git. Classify actual test output separately from fixtures and approved baselines in artifacts.outputs and artifacts.baselines. Write inspection captures under `.agenthouse/evidence/<work-id>/`. `housekeep --check` verifies effective Git exclusions and indexed output; ordinary housekeep/session preserve evidence. Only `housekeep --clean` removes explicitly configured artifacts.cleanup paths, with indexed-file and CI retention protection. Never infer ownership from screenshot or directory names. Archive the actual evaluation report folder in CI; central evaluations default to `.agenthouse/local/reports/`. See [ADR-0014](adr/0014-central-storage-and-repository-integration.md).
...
`npm run test:browser` exercises a real local fixture, establishes a test-only baseline under work/, deliberately breaks the layout, asserts failure, restores it, and verifies the evidence. After the run it deletes those local screenshots and Playwright output unless `CI` or `AH_KEEP_BROWSER_ARTIFACTS` is set. Set `AH_BROWSER_EXECUTABLE` for an already installed Chrome/Chromium, or provision the pinned Playwright browser first. This test script's baseline generation is not a production pipeline pattern. The Playwright template writes `outputDir` and the assessment JSON under `artifacts/agenthouse/`.
...
The lifecycle skill is stored centrally for new enrollments; `node .agenthouse/run.mjs context` identifies its exact path. Shared instruction bridges reference that command. Private integration needs explicit context loading. Legacy project-copy installations retain `.agents/skills/ah-lifecycle`. Native plugin/skill-loader behavior is not yet host-certified. `agenthouse-hooks` remains independent; invoke a reviewed `scan` command through an exit-code evaluator if needed. Its malformed-input `run --vendor ci` behavior still needs upstream correction before that path is authoritative.
