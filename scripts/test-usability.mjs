...
try {
  const git=spawnSync('git',['init',root],{encoding:'utf8',windowsHide:true});assert(git.status===0,git.stderr);
  install(root,{agents:[]});
  await setupUsability(root);
  for(const [name,expected] of [['pass',0],['fail',1]]) {
    const output=`artifacts/agenthouse/usability/${name}.json`;
    const r=spawnSync(process.execPath,[path.join(root,'.agenthouse/run.mjs'),'usability','run','--root',root,'--fixture',path.join(skillDirectory(root,'web-usability-conformity'),`fixtures/${name}-basic.html`),'--output',output],{encoding:'utf8',windowsHide:true,timeout:180000});
