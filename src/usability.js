...
export function usabilityRuntime(root) {
  ...
  return {skill,pkg,lock,digest,directory:inside(root,`.agenthouse/local/tools/usability-${digest.slice(0,16)}`)};
}
export async function setupUsability(root,{npmCli,offline=false}={}) {
  ...
  for(const file of Object.keys(skill.files))write(inside(directory,file),fs.readFileSync(inside(root,`.agents/skills/${skill.id}/${file}`)));
...
export async function auditUsability(root,{url,fixture,output='artifacts/agenthouse/usability/report.json'}={}) {
  ...
  const target=inside(root,output);assert(!fs.existsSync(target),'Choose a new report path to preserve earlier evidence');
  const result=await run(process.execPath,[path.join(directory,'scripts/audit.mjs'),...(url?['--url',url]:['--fixture',inside(root,fixture)]),'--out',target,'--screenshots'],{cwd:root,timeoutSeconds:180});
