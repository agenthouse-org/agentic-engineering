...
const guidance={
  ...
  process:'Use process list/show to choose product lifecycle guidance. process start prints steps, roles, outputs, and linked skills but does not update a tracker. For “where are we?”, gather current evidence, call process where, state that its result is tentative, and ask the user to compare with the authoritative status source. Consumer baseline copies require explicit adoption and update review.',
...
Screens: HTML fragments (no html/head/body/script), real product copy, one surface (browser, desktop, mobile, popover, panel). Architecture stays in mermaid (erDiagram, classDiagram, sequenceDiagram, stateDiagram-v2, flowchart) — never inside a screen. Open questions are listed decisions with a recommended option.`,
  ...
  housekeep:'Verify effective Git exclusions for configured generated output. Inspect actual test-tool output paths per repository and record outputs separately from fixtures/baselines. Never infer permission to ignore or delete directories from their names. Preserve evidence by default. Only explicitly requested housekeep --clean may remove configured artifacts.cleanup paths; Git errors or indexed files prevent deletion.',
...
const workflows={
  'assess-tech-feasibility':['Assess a proposed technical change, blast radius, encapsulation, rollout options, migration and consumer impact. Use when asking about feasibility or an architecture decision before implementation.', `This is an assessment workflow, not an evaluator or approval. Read docs/technical-feasibility.md from the pinned runtime for the executable inventory and summary contract.
...
Run relevant available checks through the configured CLI and inspect UI screenshots through frontend-acceptance when applicable. Distinguish baseline defects and missing evidence. Disclose if you authored the change. A review report does not approve, merge, or mark work done.`],
  ...
  'frontend-acceptance':['Compare UI work to the story, bug, wireframe, or reference image using real screenshots and the pinned frontend-acceptance method. Use when checking visual design, screens, or frontend acceptance.', `Run dependencies status, then run context and read its frontend-acceptance SKILL.md and follow its supporting resources. Derive evidence from the reference image, wireframe, story, or bug. Do not substitute regression equality for concept conformance. If the upstream dependency is missing or modified, report the problem rather than inventing an alternative method.
...
const descriptions={
  ...
  onboard:'Choose shared or private repository integration, coding agents, artifact locations, policy, and autonomy, then enroll this repository. Use when starting with agenthouse or adding it to a project.',
  ...
  evaluate:'Run checks configured in the repository profile against a specific build and write evidence reports; does not assess requirements, scope or design. Use when running local or CI evaluation, pull-request checks, or verifying a specific build.',
  ...
  housekeep:'Verify that configured generated output stays out of Git; preserve fixtures, baselines and review evidence. Use when leftover captures appear, after frontend-acceptance, or at session start.',
  ...
  restore:'Restore missing central assets or legacy project projections from this project’s exact recorded version. Use when the pinned runtime or skills are missing after a clone or machine change.',
  ...
  roles:'Discover and load role guidance, or adopt and review consumer-owned role baselines. Use when asking the agent to work from a role such as product manager, engineer, support, market research, or sales.',
  process:'Navigate the product lifecycle or subprocesses for features, bugs, changes, releases, and incidents, or request orientation about current evidence. Use when starting product-process guidance or asking where work stands. Process guidance does not track or change status.'
};
